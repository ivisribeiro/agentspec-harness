...
| `G_BUILD` | before `/ship` | a manifest file is missing on disk, criteria-diff non-empty, no BUILD_REPORT, a passed criterion's CI `verified_by_result` is `failed`, or (when `config.require_verified_by`) a passed criterion cites no verifier. The spine READS the CI result; it never runs the verifier |
