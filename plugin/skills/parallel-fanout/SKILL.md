...
description: |
  Explains and enforces the ahx parallel fan-out pattern: when to parallelize vs sequence work,
...
# Parallel Fan-Out in the ahx Harness

The harness has one invariant: `ahx` makes every ordering and gate decision; Claude only
authors artifacts and fans out workers. Parallel execution is a direct consequence of
`ahx next` returning more than one artifact in the same `parallel_group`.

---

## Lane Matrix

| Lane | When to use | `ahx` behavior |
|------|-------------|----------------|
| **Parallel** | Artifacts are independent — no dependency edge between them | `ahx next` returns all of them in the same `parallel_group`; dispatch all in a SINGLE message |
| **Sequential** | Artifact B reads the output of artifact A | `ahx next` returns A alone; B appears only after `ahx complete A` succeeds |
| **Retry loop** | A worker's handoff fails `ahx complete --handoff` validation | `ahx retry <id> --inc` increments the counter; `--ok` exits 1 at the `build_retry_cap` ceiling — stops the loop |
...
- `ahx next` places them in the same `parallel_group`.
...
- `ahx next` returns only one artifact (`parallel_group` is absent or unique).

Never infer dependency from topic similarity alone — trust `ahx next` exclusively.

---

## Single-Message Task Fan-Out

`ahx next` returns a ready list. When two or more items share the same `parallel_group`,
...
ahx route define-intent          # -> { tier: "opus", model: "...", reason: "..." }
ahx route spec-authoring         # -> { tier: "sonnet", model: "...", reason: "..." }
```

```
# Step 3 — fan out: dispatch BOTH workers in ONE message via Task
#   Worker A writes .ahx/features/<feature>/DEFINE.md + .ahx/features/<feature>/.handoffs/define.json
#   Worker B writes .ahx/features/<feature>/BRAINSTORM.md + .ahx/features/<feature>/.handoffs/brainstorm.json
```
...
# exit 1 -> handoff invalid: call ahx retry DEFINE --inc (re-dispatch bounded by --ok ceiling)

ahx complete BRAINSTORM --handoff .ahx/features/<feature>/.handoffs/brainstorm.json
```
...
Never call `ahx complete` without `--handoff` when a handoff schema applies.
Never call `ahx gate` before every artifact in the group is complete.
...
`ahx complete <id> --handoff <file>` validates the sidecar against the schema before
...
```bash
ahx handoff-check define .ahx/features/my-feature/.handoffs/define.json
...
**Isolation rules:**
1. Run `ahx complete` independently for each artifact. A failure on one does not abort others.
2. `ahx retry <id> --inc` increments the retry counter for that artifact only.
3. `ahx retry <id> --ok` exits 1 (stop re-dispatch) when the counter reaches `build_retry_cap` — surface the failure to the user.
4. Run `ahx gate <gateId>` only after ALL artifacts in the group have reached a terminal state
...
ahx retry BUILD_FOO --inc   # exit 0 -> re-dispatch worker
ahx retry BUILD_FOO --inc   # exit 0 -> re-dispatch worker
ahx retry BUILD_FOO --ok    # exit 1 at ceiling -> surface to user, do not re-dispatch
```
...
Run `ahx route <kind>` for EACH artifact before dispatching. Independent artifacts in
...
```
/define  ->  ahx gate G_DEFINE  (DEFINE sections + AC-n ids + define handoff valid)
/design  ->  ahx gate G_DESIGN  (manifest table + design handoff)
/build   ->  ahx gate G_BUILD   (every manifest file on disk + criteria-diff empty + BUILD_REPORT)
/ship    ->  ahx gate G_SHIP    (define.criteria minus build.passed must be empty)
/review  ->  ahx gate G_REVIEW_BLOCK  (surviving CRITICAL findings > 0 -> block)
```

Check unmet criteria between phases:

```bash
ahx diff-criteria --define .ahx/features/my-feature/DEFINE.md \
                  --build  .ahx/features/my-feature/BUILD_REPORT.md
...
| Calling `ahx complete <id>` without `--handoff` | Skips schema validation; G_HANDOFF gate will block | Always pass `--handoff <sidecar>` when a handoff schema applies |
| Advancing to the next phase before calling `ahx gate` | Skips the deterministic gate; may ship invalid artifacts | Call the gate after every phase; branch strictly on exit code |
| Retrying indefinitely without `ahx retry --ok` | Infinite loop on permanent failures | Use `--inc` / `--ok` to respect `build_retry_cap` |
| Inferring dependency from topic similarity | Wrong sequencing | Trust `ahx next` and `parallel_group` exclusively |
