...
1. Fetch the feature slug from run.json (via `ahx state`)
2. Glob `.ahx/features/{feature}/*.md` to find the DEFINE and BUILD_REPORT files
...
7. Write `.ahx/features/{feature}/SHIPPED.md`
8. Write `.ahx/features/{feature}/LESSONS.md`
9. Write `.ahx/features/{feature}/.handoffs/{id}.json` (handoff: build-report schema)
10. Call `ahx complete <id> --handoff <sidecar>`
...
```json
{
  ...
  "criteria_count": 12,
  "criteria_passed": 12,
  "artifacts": [".ahx/features/<feature>/SHIPPED.md", ".ahx/features/<feature>/LESSONS.md"],
...
- Do NOT invent new ahx commands or flags. Use only those listed in _authoring_context.md.
- Do NOT check external systems or call models. This is deterministic prose synthesis.
- Do NOT modify any .ahx file except SHIPPED.md and the .handoffs/*.json sidecar.
