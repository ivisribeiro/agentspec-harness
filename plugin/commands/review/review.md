...
> `.ahx/review/arch-findings.json`
>
> Then write a handoff sidecar to:
> `.ahx/review/.handoffs/arch.json`
...
>
> Write a JSON array of all findings to:
> `.ahx/review/security-findings.json`
>
> Then write a handoff sidecar to:
> `.ahx/review/.handoffs/security.json`
> matching the `finding` handoff schema:
> `{ "findings": [ ...same array... ] }`
...
```
node ${CLAUDE_PLUGIN_ROOT}/dist/cli/index.js handoff-check finding .ahx/review/.handoffs/arch.json
```
...
```
node ${CLAUDE_PLUGIN_ROOT}/dist/cli/index.js handoff-check finding .ahx/review/.handoffs/security.json
```
...
Read `.ahx/review/arch-findings.json` and `.ahx/review/security-findings.json`. Merge into a single list, deduplicating by `(file, line)` — when two findings share the same file+line keep the one with the higher severity (critical > high > medium > low); if equal severity, keep the more detailed `detail`. Write the merged list to `.ahx/review/merged-findings.json`.
...
From `.ahx/review/merged-findings.json`, collect all findings where `severity == "critical"`. If there are none, skip to step 8.
...
- Instructions:
  ...
  > `.ahx/review/.handoffs/adversary-<slug>.json`
  > where `<slug>` is a short sanitized form of `finding_key`.

### 8. Apply adversary verdicts

Read every `.ahx/review/.handoffs/adversary-*.json` produced in step 7. For each finding in the merged list:
...
Write the final annotated list to `findings.json` (project root or the path `ahx gate G_REVIEW_BLOCK` reads via `--findings`).
